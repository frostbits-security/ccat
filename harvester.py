...
reg_secret=r"enable secret (\d).+\b([\w\$]+)"
reg_username_pass=r"username ([\w]+).+password (\d) \b(.+)"
reg_username_secret=r"username ([\w]+).+secret (\d) \b(.+)"
...
with open('usernames.json', 'w') as outfile:
	json.dump(list(set(usernames)), outfile)
with open('passwords.json', 'w') as outfile:
	json.dump(passwords, outfile)
with open('secrets.json', 'w') as outfile:
	json.dump(list(set(secrets)), outfile)
with open('clears.json', 'w') as outfile:
	json.dump(clears, outfile)