...
# Todo:  
- доделать парсинг  
- закомментировать всё по максимуму  
- пихнуть все проверки в одну папку  
- унифицировать вывод  
- унифицировать принты  
- дальше делать проверки  
- сделать список проверок  

Вывод проверки в принт: ['check_name',severity(int),'message','best practice']  
...
- Проверка настроек RADIUS, TACACAS
- OSPF, EGRP
- HSRP
...
информации об этом нет. Если включено, то
требуется создание vtp domain, password (отсутствует упоминание в конфиге;
mode,domain только в режиме off, transparent)
* DTP (default, local)
Динамическая настройка транков. Отключение:
`switchport nonegotiate`
* Storm-control (local)
Предотвращение широковещательного шторма.
Могут быть указаны два параметра: 1-порог, после которого срабатывает storm-control, 2-*приемлемый* уровень. Может быть указан один параметр-число, являющееся количеством приемлемого трафика, выраженным в процентах. 
* STP (default, global)
Устранение/предотвращение петель.
Защита:
    - The Portfast feature, which disables spanning tree on a
port.
    - The Root/BPDU guard feature, Loop Guard (предотвращает петли)
* CDP (default, global/local)
Информация об устройствах, подключенных к интерфейсам 
Отключить на интерфейсе:
`no cdp enable` 
Отключить глобально:
`no cdp run`
* syslog(global)
