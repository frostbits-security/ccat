    ...
    if 'port-security' in iface_dct and len(iface_dct['port-security']) != 0:
        ...
        if 'violation' in port_sec_dct:
            ...
            if port_sec_dct['violation'][0] in violation:
                ...
                result.update({'Port-security violation': [scale[1], 'WARNING', "Port-security violation should be 'restrict' or 'shutdown'"]})
        else:
            result.update({'Port-security violation': [scale[0], 'DISABLED',
