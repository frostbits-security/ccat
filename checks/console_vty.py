    ...
    for line in global_params['line']:
        ...
        if 'log_sync' in global_params['line'][line]:
            if global_params['line'][line]['log_sync'] == 'yes':
                ...
                results_dict['Lines'][line]['logging synchronous'] = [1,'DISABLED', 'You may want to enable it to prevent '
                                                                                    'command typing interrupt']
        if global_params['line'][line]['no_exec'] == 'yes':
            ...
            if 'exec_timeout' in global_params['line'][line]:
                if   0  < global_params['line'][line]['exec_timeout'] < 15:
                    ...
                    results_dict['Lines'][line]['exec timeout'] = [1, str(global_params['line'][line]['exec_timeout']),
                                                                  'You may want to decrease it']
                elif      global_params['line'][line]['exec_timeout'] > 30:
                    results_dict['Lines'][line]['exec timeout'] = [0, str(global_params['line'][line]['exec_timeout']),
            ...
            if 'privilege' in global_params['line'][line]:
                if global_params['line'][line]['privilege'] == '15':
                    results_dict['Lines'][line]['privilege level'] = [1, 'High privilege level',
                                                                      'You may want to decrease it']
